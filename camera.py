...
def predict_and_update(frame):
    ...
    with prediction_lock:
        ...
        predicting = True
    try:
        # ✅ Run prediction on correct device
...
def update_frame():
    global last_pred_time, camera_ready, tk_image
    ret, frame = cap.read()
    if ret:
        if not camera_ready:
            camera_ready = True
        ...
        show_logo()
    root.after(100, update_frame)  # ~10 FPS
...
tk.Label(dashboard, text="\U0001F9E0 Detection Summary", font=("Arial", 14, "bold"), fg="white", bg="#2E2E2E").pack(pady=(0,10))
...
tk.Button(dashboard, text="\U0001F4F8 Capture Image", font=("Arial", 10), command=capture_image,
          bg="#1E90FF", fg="white", relief="flat", padx=8, pady=4, width=14).pack(pady=6)
tk.Button(dashboard, text="\U0001F4C2 Open Results Folder", font=("Arial", 10), command=open_results_folder,
...
tk.Button(bottom_frame, text="\U0001F7E2 Start Detecting", font=("Arial",10,"bold"), command=start_detection,
          bg="#32CD32", fg="white", relief="flat", padx=12, pady=6, width=14).grid(row=0,column=0,padx=5)
tk.Button(bottom_frame, text="\U0001F6A9 Stop Detecting", font=("Arial",10), command=stop_detection,
